...
//            LogUtils.e("nativeSetPreviewSize" + result);
            if (result != 0)
...
//        final int type = (mCurrentFrameFormat > 0) ? 6 : 4;
    ...
    public int nativeXuWrite(int cmd, int addr, int len, byte[] pbuf)
    {
        if (mControlBlock != null) {
            return native_xu_write(mNativePtr, cmd, addr, len, pbuf);
        } else {
            return -1;
        }
    }
    ...
    {
        if (mControlBlock != null) {
            return native_xu_read(mNativePtr, cmd, addr, len, pbuf);
        } else {
            return -1;
        }
    }
