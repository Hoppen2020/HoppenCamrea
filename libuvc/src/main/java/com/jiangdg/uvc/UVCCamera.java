...
//	private UsbControlBlock mCtrlBlock;
	 ...
	 * @param controlBlock
	 */
	public synchronized void open(final ControlBlock controlBlock) {
		int result;
		try {
...
//**********************************************************************
	...
	// qzm
	private static native int nativeXuWrite(final long id_camera, int cmd, int addr, int len, byte [] pbuf);
	private static native int nativeXuRead(final long id_camera, int cmd, int addr, int len, byte [] pbuf);


	public int jXuWrite(int cmd, int addr, int len, byte[] pbuf)
	{
		if (mControlBlock != null) {
			return nativeXuWrite(mNativePtr, cmd, addr, len, pbuf);
		} else {
			return -1;
		}
	}

	public int jXuRead(int cmd, int addr, int len, byte[] pbuf)
	{
		if (mControlBlock != null) {
			return nativeXuRead(mNativePtr, cmd, addr, len, pbuf);
		} else {
			return -1;
		}
	}
