...
// qzm
static jint nativeXuWrite(JNIEnv *env, jobject thiz, ID_TYPE id_camera,
							jint cmd, jint addr, jint len, jbyteArray pbuf)
{
	jint result = JNI_ERR;
	...
	if (LIKELY(camera)) {
		...
		unsigned char *pbuffer = (unsigned char*)env->GetByteArrayElements(pbuf, JNI_FALSE);
		if (pbuffer != NULL) {
			result = camera->xuWrite((int)cmd, (int)addr, (int)len, pbuffer);
			env->ReleaseByteArrayElements(pbuf, (jbyte*)pbuffer, JNI_FALSE);
		}
...
static jint nativeXuRead(JNIEnv *env, jobject thiz, ID_TYPE id_camera,
						   jint cmd, jint addr, jint len, jbyteArray pbuf)
{
	jint result = JNI_ERR;
	...
	if (LIKELY(camera)) {
		...
		unsigned char *pbuffer = (unsigned char*)env->GetByteArrayElements(pbuf, JNI_FALSE);
		if (pbuffer != NULL) {
			result = camera->xuRead((int)cmd, (int)addr, (int)len, pbuffer);
			env->ReleaseByteArrayElements(pbuf, (jbyte*)pbuffer, JNI_FALSE);
		}
...
static JNINativeMethod methods[] = {
	// qzm
	{ "nativeXuWrite",     "(JIII[B)I", (void *) nativeXuWrite },
	{ "nativeXuRead",      "(JIII[B)I", (void *) nativeXuRead },
